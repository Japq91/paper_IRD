...
# ============================================================================
var = 't2m'  # 'pr' = precipitación, 't2m' = temperatura
outfile = f'../figures/test1_{var}.png'

SHP_RUTA = '../input_shp'
CSV_RUTA = '../input_csv'
OBS_RUTA = '../data/obs_mean_files'
NC_RUTA = '../data/exp_mean_files'

files=[f'{NC_RUTA}/{var}/prom_EXP0.nc', f'{NC_RUTA}/{var}/prom_EXP1.nc', f'{NC_RUTA}/{var}/prom_EXP5.nc', 
       f'{NC_RUTA}/{var}/prom_EXP8.nc', f'{NC_RUTA}/{var}/prom_EXP10.nc', f'{CSV_RUTA}/{var}_all_experimentos.csv',
       f'{NC_RUTA}/{var}/anom_EXP1.nc', f'{NC_RUTA}/{var}/anom_EXP5.nc', f'{NC_RUTA}/{var}/anom_EXP8.nc',
       f'{NC_RUTA}/{var}/anom_EXP10.nc',
       ]
...
exps2=['GMTs\nMOD30_SOM','GMTns\nMOD30_SOM','GMTns\nEVA_SOM','GMTns\nEVA_SIE',
       'GMTns\nEVA_SIE_SST','Station Data', 'b) - a)','c) - b)','d) - c)','e) - d)',
       ]
# ============================================================================
...
def get_cmap_norm(filepath):
    ...
    if 'anom' in filepath:
        ...
        bounds = np.array([1,3,5,7,9,9.5,10,10.5,11,11.5,12,12.5,13,15,20,25,35])
...
gs = gridspec.GridSpec(2, 5, figure=fig, wspace=0.05, hspace=0.04)
...
for i in range(2):
    for j in range(5):
        ...
        if (i, j) == (0, 4) and im is not None:
            cax = fig.add_axes([0.91, 0.54, 0.02, 0.3])
            cbar = fig.colorbar(im, cax=cax, ticks=bounds[1:-1][::2],
                                label="[°C]", extend='both')
        if (i, j) == (1, 4) and im_anom is not None:
            cax = fig.add_axes([0.91, 0.15, 0.02, 0.3])
